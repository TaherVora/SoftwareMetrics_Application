...
from pyui.uucw_dialog import Ui_Dialog

class UucwDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        super(UucwDialog, self).__init__()
        self.ui = Ui_Dialog()
        ...
        for i in range(1, self.ui.gridLayout.rowCount()):
            for j in [2, 3, 4]:
                try:
                    ...
                    if j == 2 and isinstance(widget, QtWidgets.QLabel):
                        self.widget_dict[str(j)][str(i)] = widget
                    if j == 3 and isinstance(widget, QtWidgets.QLineEdit):
                        self.widget_dict[str(j)][str(i)] = widget
            ...
            if isinstance(widget_result, QtWidgets.QLabel):
                try:
                    widget_result.setText(
                        str(float(widget_weight.text()) * float(widget_input.text()))
                    )
                except:
                    pass
        try:
            ...
            for i in range(1, self.ui.gridLayout.rowCount()-1):
                try:
                    ...
                    pass
            self.parent.ui.label_uucw_result.setText(str(total_sum))
    ...
    def save(self):
        ...
        self.parent.parent.project_data_dict[self.parent.parent.file_new_project_dialog.project_name]["UCP"][
            self.parent.ucp_filename]["uucw"] = self.adj_value_dict
